...
function setCstmParaVal(pageid,reportid,paraname,paraval){
	setCustomizeParamValue(pageid,reportid,paraname,paraval);
}
/**
 * 查询API
 * pageid 
...
function setHiddenCols2SelectedRow(data, p, trobj){
	if(trobj == null){
		...
		for (var item in data) {
			...
			var val = data[item];alert(val);
			var tdobj = p.$(trobj).find("#"+keytdid);
			var chval = tdobj.attr("value");
			if(chval != null){
				tdobj.attr("value",val);
				continue;
			}
			var oldname = tdobj.attr("oldvalue_name");
			var oldval = tdobj.attr("oldvalue");
			if(oldval == null)tdobj.attr("oldvalue",val);
			if(tdobj.attr("value") != null){
				tdobj.attr("value",val);
			}else{
			...
			if(tdobj.children()==0)
			{
				tdobj.append('<div style="width:100%" class="cls-data-content-list"><span tagtype="COL_CONTENT_WRAP"><input name="'+inputid+'" class="cls-inputbox2-full" id="'+inputid+'" style="text-align: left;" onkeypress="return onKeyEvent(event);" onfocus="try{this.select();}catch(e){logErrorsAsJsFileLoad(e);}" onblur="try{try{addInputboxDataForSaving(\''+guid+'\',this);}catch(e){logErrorsAsJsFileLoad(e);};fillInputBoxValueToParentTd(this,\'textbox\',\''+guid+'\',\'editablelist2\',1);}catch(e){logErrorsAsJsFileLoad(e);}" type="text" typename="textbox" value="'+val+'"/></span></div>');
			}
		}
	}
}
...
/**
 * 产生uuid编号，并给单据id字段复制
 * 这里建议设计主表的时候一定要有一个id字段，这个id字段是uuid的，如果需要可以另加字段作为联合主键，来表示单据含义！
 */
function generalid(pageid, reportid){
	//查看是否设定了转换id隐藏域
	/*var accessmode = getEditableReportColValues(pageid, reportid, {pt_id:true});
	alert(accessmode.pt_id.value);
	if(accessmode != 'add')return;*/
	var djid = 'id';
	var idname = $('#idkey').val();
	if(idname != null)djid = idname;
	var idval = $.jbsf.util.uuidFast();
	setInputboxValueForDetailReport(pageid, reportid, djid, idval);
}

/**
 * 获得指定条件组件的编号
 */
function getCdtGuid(pageid, reportid, cdtid){
	var reportguid = getComponentGuidById(pageid, reportid);
	var cdtguid = reportguid + "_condition_" + cdtid;
	return cdtguid;
}
 ...
 * 获得指定可编辑报表可编辑组件的guid
 */
function getcomGuid(pageid, reportid, comid){
	var reportguid = getComponentGuidById(pageid, reportid);
	return reportguid+"_"+comid;
}