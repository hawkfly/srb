...
//            }
        ...
        getLang: function (path) {
            ...
            //document.write("lang = " + lang + ", options.lang = " + this.options.lang);
            if (!lang) {
...
//                img.style.cssText += ';margin:' + (first.margin||0) +"px;" + 'border:' + (first.border||0) +"px solid #000";
            ...
            if (opt.length == 1) {
            	ci = opt[0];
                if(ci.title != null){
                	var title = ci.title.lastIndexOf('.') == -1 ? ci.title : ci.title.substring(0, ci.title.lastIndexOf('.'));
                	var titlep = '<p class="imgtitlespan">'+title+'</p>';
                }
                ...
                html.push(str + titlep);
            } else {
                for (var i = 0; ci = opt[i++];) {
                    if(ci.title != null){
                    	var title = ci.title.lastIndexOf('.') == -1 ? ci.title : ci.title.substring(0, ci.title.lastIndexOf('.'));
                    	var titlel = '<p class="imgtitlespan">'+title+'</p>';
                    }
...
UE.commands['indent'] = {
    execCommand : function() {
         var me = this,value = me.queryCommandState("indent") ? "0em" : (me.options.indentValue || '32px');
                    ...
                    for (var i = 0, ci; ci = imgs[i++];) {
                        ...
                        for (var j = 0, cj; cj = srcUrls[j++];) {
                            ...
                            if (src == cj && url != "error") {  //抓取失败时不做替换处理
                                //地址修正
                            	if(url == null || url == 'null')continue;//hawkfly 2013-09-10
...
///import core
///import plugins/ysyd.js
///commands 延伸阅读自定义组件
///commandsName ysyd
///commandsTitle  延伸阅读
///commandsDialog  dialogs\ysyd
UE.commands['ysyd'] = {
	execCommand : function(){
		var me = this;
		art.dialog.open($.jbsf.core.contextpath()+'/jbsf.sr?PAGEID=expandreadPage',
			{
			 id:'ysyd',
			 name:'ysyd',
			 title:'延伸阅读',
			 ok: function(){
				 var result = "<ul>";
				 parent.frames['Openysyd'].$("#to option").each(function(i){//将选中的所有延伸阅读项拼接为有效html格式
					 var url = '#';
					 if(this.title == '1'){//学院主站
						 url = 'http://www.sdcmc.net/sdcmw/xyzz/view.do?id=' + this.value;
					 }else if(this.title == '2'){//招生信息网
						 url = 'http://www.sdcmc.net/sdcmw/zsw/view.do?id=' + this.value;
					 }else if(this.title == '3'){//播音主持系
						 url = 'http://www.sdcmc.net/sdcmw/xbzd/byzcx/view.do?id=' + this.value;
					 }else if(this.title == '4'){//影视艺术系
						 url = 'http://www.sdcmc.net/sdcmw/xbzd/ysys/view.do?id=' + this.value;
					 }else if(this.title == '5'){//新闻传播系
						 url = 'http://www.sdcmc.net/sdcmw/xbzd/xwcb/view.do?id=' + this.value;
					 }else if(this.title == '6'){//信息工程系
						 url = 'http://www.sdcmc.net/sdcmw/xbzd/xxgc/view.do?id=' + this.value;
					 }else if(this.title == '7'){//动画系
						 url = 'http://www.sdcmc.net/sdcmw/xbzd/dh/view.do?id=' + this.value;
					 }else{//各部门
						 url = 'http://www.sdcmc.net:8080/sdcmw/dep/CreatView.do?depid='+this.title+'&mid='+this.lang+'&id='+this.value;
					 }
					 var li = "<li><a href='"+url+"'>"+$(this).text()+"</li>";
					 result += li;
				 });
				 result += "</ul>";
				 if(result != '<ul></ul>'){
					 me.execCommand("inserthtml","<br/>"+result);
				 }
			     return true;
			 }
			});
...
(function (){
    ...
    Toolbar.prototype = {
        ...
        getHtmlTpl: function (){
            ...
            if(UEDITOR_CONFIG.isFirstOthersToolbar){
            	if(this['tbnum'] == 0){
            		return '<div id="##" class="edui-toolbar %%" onselectstart="return false;" onmousedown="return $$._onMouseDown(event, this);">' +
		            		buff.join('') +
		            		'</div>'
            	}else{
            		return '<div id="##" class="edui-toolbar hidden %%" onselectstart="return false;" onmousedown="return $$._onMouseDown(event, this);">' +
                    		buff.join('') +
                    		'</div>'
            	}
            }else{
...
(function () {
    ...
    var btnCmds = ['undo', 'redo', 'formatmatch',
        ...
        'deletecol', 'splittorows', 'splittocols', 'splittocells', 'mergecells', 'deletetable','ysyd','expand'];
...
(function () {
    ...
    EditorUI.prototype = {
        ...
        _initToolbars:function () {
            ...
            for (var i = 0; i < toolbars.length; i++) {
                ...
                var toolbarUi = new baidu.editor.ui.Toolbar({theme:editor.options.theme, tbnum:i});
    ...
    UE.init = function (opt) {
	    var editor = $.jbsf.wbcs.editor = new UE.ui.Editor(opt.options);
	    editor.render(opt.renderid);
	    editor.addListener('selectionchange',function(){
	        eval(opt.onchange);
	    });
    }
