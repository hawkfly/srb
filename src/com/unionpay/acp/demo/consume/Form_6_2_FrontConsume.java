...
import com.pansoft.service.SRBPayService;
...
public class Form_6_2_FrontConsume extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		String merId = "802370048160534";
		String txnAmt = req.getParameter("txnAmt");
		String orderid = DemoBase.getOrderId();
		String fqtime = DemoBase.getCurrentTime();
		String cstmname = req.getParameter("cstmname");
		String pdtid = req.getParameter("pdtid");
		
		String accmoney = req.getParameter("accmoney");
		String cjmoney = req.getParameter("cjmoney");
		if(Utils.isNull(pdtid)){
			resp.getWriter().write(Consts.PREPAYERROR);
			String msg = Utils.margeString("待付款产品ID为空值，阻止付款！");
			JbsfLogUtils.error(Consts.PAYPREACTIONE, msg);
			return;
		}
		
		if(Utils.isNull(cstmname))cstmname = "hawkfly";
		
		SRBPayService service = new SRBPayService();
		if(!service.preAction(cstmname, orderid, pdtid, fqtime, accmoney, cjmoney)){
			resp.getWriter().write(Consts.PREPAYERROR);
			String msg = Utils.margeString("用户 ", cstmname, " 付款 "+txnAmt +"分  记录订单号失败，阻止交易继续执行！");
			JbsfLogUtils.error(Consts.PAYPREACTIONE, msg);
			return;
		}
		...
		requestData.put("channelType", "08");           			  //渠道类型】，这个字段区分B2C网关支付和手机wap支付；07：PC,08：手机，平板等
		
		/***商户接入参数***/
		requestData.put("merId", merId);    	          			  //商户号码，请改成自己申请的正式商户号或者open上注册得来的777测试商户号
		requestData.put("accessType", "0");             			  //接入类型，0：直连商户 
		requestData.put("orderId", orderid);             //商户订单号，8-40位数字字母，不能含“-”或“_”，可以自行定制规则		
		requestData.put("txnTime", fqtime);        //订单发送时间，取系统时间，格式为YYYYMMDDhhmmss，必须取当前时间，否则会报txnTime无效
		requestData.put("currencyCode", "156");         			  //交易币种（境内商户一般是156 人民币）		
		requestData.put("txnAmt", txnAmt);             			      //交易金额，单位分，不要带小数点
		requestData.put("reqReserved", pdtid);        		      //请求方保留域，透传字段（可以实现商户自定义参数的追踪）本交易的后台通知,对本交易的交易状态查询交易、对账文件中均会原样返回，商户可以按需上传，长度为1-1024个字节		
